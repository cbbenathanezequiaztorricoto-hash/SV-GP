...
async function cargarReportes() {
    // Cargar selects de clientes, productos, métodos de pago
    await cargarSelectsReportes();
    // Ejecutar búsqueda por defecto (últimos 30 días)
    const hoy = new Date();
...
async function generarReporte() {
    ...
    try {
        const res = await fetch(`/api/ventas/reportes/ventas?${params.toString()}`);
        ...
        tbody.innerHTML = '<tr><td colspan="7" style="color:red;text-align:center;">Error al cargar reporte.</td></tr>';
    }
}
...
async function descargarReporteExcelAvanzado() {
    ...
    try {
        const res = await fetch(`/api/ventas/reportes/excel-avanzado?${params.toString()}`);
...
// Función para reporte por cliente (opcional)
async function cargarReporteClientes() {
    const tbody = document.getElementById('tablaReporteClientesCuerpo');
    if (!tbody) return;
    ...
    try {
        const res = await fetch(`/api/ventas/reportes/clientes?${params.toString()}`);
        const data = await res.json();
        if (data.length === 0) {
        ...
        tbody.innerHTML = data.map(row => `
            <tr>
                <td>${row.cliente}</td>
                <td>${row.compras}</td>
                <td style="font-weight:bold;">${parseFloat(row.totalGastado).toFixed(2)} Bs.</td>
                <td><button class="btn-submit btn-sm" onclick="alert('Ver detalle de cliente ${row.idCliente}')">Ver detalle</button></td>
            </tr>
        `).join('');
    } catch {
        tbody.innerHTML = '<tr><td colspan="4" style="color:red;text-align:center;">Error.</td></tr>';
    }
}